...
export default function MenuPreview({ 
  ...
  return (
    <section id="menu" className="relative py-32 px-6 bg-black-card">
      {/* Background texture */}
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto">
          ...
          >
            {locations.map((location, index) => (
              <button
                ...
                className={`relative px-8 py-4 font-playfair text-lg tracking-wider transition-all duration-300 rounded-lg ${
        ...
        >
          {currentLocationData.map((category, index) => (
            <button
              key={category.name}
        ...
        {/* Menu Items Grid */}
        {currentLocationData.length > 0 && currentLocationData[activeCategory] && (
          <motion.div
            key={`${activeLocation}-${activeCategory}`}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="grid md:grid-cols-2 gap-8"
          >
            {currentLocationData[activeCategory]?.items?.map((item, index) => (
              <motion.div
                key={item.name}
                initial={{ opacity: 0, y: 30 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
