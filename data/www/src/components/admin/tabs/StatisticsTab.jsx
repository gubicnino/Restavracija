...
export default function StatisticsTab() {
  ...
  const calculateStats = reservations => {
    ...
    const thisMonth = reservations.filter(r => new Date(r.datum) >= startOfMonth).length;

    const thisWeek = reservations.filter(r => new Date(r.datum) >= startOfWeek).length;

    const avgGuests =
      reservations.length > 0
        ? (
            reservations.reduce((sum, r) => sum + parseInt(r.stevilo_oseb), 0) / reservations.length
          ).toFixed(1)
        : 0;
    ...
    const topTimeSlot = Object.keys(timeSlots).reduce(
    ...
    const topDay = Object.keys(days).reduce((a, b) => (days[a] > days[b] ? a : b), '');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KeyInsights icon={<Award />} title="Najbolj priljubljen čas" value={stats.topTimeSlot} />
        <KeyInsights icon={<Calendar />} title="Najbolj zaseden dan" value={stats.topDay} />
